...
public class SQLService {
    static ConnectionPool connectionPool = new BasicConnectionPool();
    Gson gson = new Gson();

    public SQLService(BasicConnectionPool basicConnectionPool) {
    ...
    public CustomArrayList<Customer> getCustomers(String id) {
        try {
            ...
            if (!id.equals("all")) {
                myStmt = connectionPool.getConnection().prepareStatement("SELECT * FROM customers WHERE id = ?");
                myStmt.setInt(1, Integer.parseInt(id));
            } else {
                myStmt = connectionPool.getConnection().prepareStatement("SELECT * FROM customers");
            }
    ...
    public CustomArrayList<Order> getOrderById(String id) {
        try {
            PreparedStatement myStmt = connectionPool.getConnection().prepareStatement("SELECT * FROM orders o JOIN customers c ON c.id = o.customer_id WHERE o.id = ?");
            myStmt.setInt(1,Integer.parseInt(id));
            ResultSet rs = myStmt.executeQuery();
            return getOrders(rs);
        } catch (SQLException e) {
    ...
    public CustomArrayList<Product> getProductsByCustomerId(String customerId) {
        try {
            PreparedStatement myStmt = connectionPool.getConnection().prepareStatement("SELECT p.id, p.name, p.price FROM public.customers c \n" +
                    "JOIN orders o ON o.customer_id = c.id\n" +
                    "JOIN order_items oi ON oi.order_id = o.id\n" +
                    "JOIN products p ON p.id = oi.product_id\n" +
                    "WHERE c.id = ?");
            myStmt.setInt(1, Integer.parseInt(customerId));
            ResultSet rs = myStmt.executeQuery();
